...
use FAlite;
use Getopt::Std;
use vars qw($opt_w $opt_s $opt_r $opt_g $opt_d $opt_a $opt_c $opt_f $opt_o $opt_m $opt_p $opt_i);
...
options:
  ...
  -i <pattern> ignore kmers from the scoring process if they match pattern
...
while (my $entry = $fasta->nextEntry) {
	...
	for (my $i = 0; $i <= length($seq) - $wordsize; $i++){
		...
		# will give zero counts to subseqs that match $ignore pattern
		if(defined $model->{$subseq} and $subseq !~ m/$ignore/){
		   	$records[$i]{score}  = $model->{$subseq};			
		} else{
		   	$records[$i]{score}  = 0;
		}

		$imeter_v1_score += $records[$i]{score} unless ($opt_i and $subseq =~ m/$ignore/);
